...
function generateSourceFiles(sourceFiles, encoding) {
    ...
    Object.keys(sourceFiles).forEach(function(file) {
        ...
        try {
            var filePath = sourceFiles[file].resolved;
            source = {
                kind: 'source',
	            mtime: fs.statSync(filePath).mtime,
                code: helper.htmlsafe( fs.readFileSync(filePath, encoding) )
            };
        }
        catch (e) {
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    data().each(function(doclet) {
        if (doclet.requires) {
	        var _rgx = /^module\:/;
	        doclet.requires.forEach(function(link) {
	            if (_rgx.test(link)) {
		            link = link.replace(_rgx, '');
	                if (!includedIn.hasOwnProperty(link)) {
		                includedIn[link] = [];
                    }
		            includedIn[link].push(doclet.longname);
                } else if (!/^\{@link .*http/.test(link)) {
	                link = link.slice(7,-1);
		            if (!includedIn.hasOwnProperty(link)) {
			            includedIn[link] = [];
		            }
